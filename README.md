...
根据测试及使用反馈，我们对此工具进行了一些优化改进，与之前版本相比，在大批量文件同步时改进后版本表现更加优秀。

从界面上看，主要更新表现在以下几个方面。

![6-01](docs/imgs/6-01.png)

选择目标空间后，目标空间所在机房会主动显示出来

开始同步之前有一个确认设置的步骤，主要进行一些基本的检查、搜索待上传的文件等准备工作

![6-02](docs/imgs/6-02.png)

默认开启“增量同步”“使用短文件名”

上传方式选择更加简洁（直传/CDN）

![6-03](docs/imgs/6-03.png)

设置完成后，点击下方[确认设置]按钮，片刻后会列出待上传的文件信息，以便做最后的检查

如果带同步文件夹下的文件过多，那么遍历文件夹会消耗一定时间，造成界面假死现象（但时间很短），这好似正常的。

确认上传文件无误后就可以[开始同步]

从逻辑上看，有一些优化细节，主要包括两方面。

一方面是SQLite数据库操作，由之前的逐条操作变更为批量操作(事务方式)

另一方面是从云端获取文件hash的时候，改用batch操作(注意文件数>1000时需要分多次batch)
